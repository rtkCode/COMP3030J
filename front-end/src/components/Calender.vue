...
<template>
    <div class="calendar">
        ...
        <tr v-for="(day,k1) in days" style="{'animation-delay',(k1*30)+'ms'}" :key="k1">
            <td v-for="(child,k2) in day" :class="{'selected':child.selected,'disabled':child.disabled}" :key="k2" @click="select(k1,k2,$event)">
        ...
        <div class="calendar-years" :class="{'show':yearsShow}">
            <span v-for="(y,index) in years" @click.stop="selectYear(y)" :key="index" :class="{'active':y==year}">{{y}}</span>
        </div>
